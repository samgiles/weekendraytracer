...
//  (-B ± sqrt(B^2 - 4AC)) / 2A
fn hit_sphere_at_distance(center: Vector4, radius: f32, ray: &Ray) -> f32 {
...
fn colour(ray: &Ray) -> Vector4 {
    let distance = hit_sphere_at_distance(Vector4::new3(0.0, 0.0, -1.0), 0.5, ray);
    if distance > 0.0 {
        // Get the surface normal
        let normal = (ray.point_at_distance(distance) - Vector4::new3(0.0, 0.0, -1.0)).unit_vector();
        // Normalize to a value between 0.0 and 1.0 to display as RGB
